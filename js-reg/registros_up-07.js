...
const ADSCRIPCION_AREA = "UP-07 DIRECCIÓN GENERAL DE ATENCIÓN AL MIGRANTE";   // ⚠️ CAMBIAR
...
function abrirModalSPG(codigo) {
  ...
  document.getElementById("spgUP").value     = "07";
...
async function abrirModalOficio2(codigo) {
  try {
    ...
    try {
      const responseOficios = await fetch(`${API}/api/presupuestos/ultimo-oficio/7`);
...
async function fetchRegistro(codigo) {
  ...
  const response = await fetch(`${API}/api/registros/UP-07`);
...
function construirOficioTablaDatos(numeroOficio) {
  return `SEBISO/SSDSYH/DGAM/${numeroOficio || ""}/2026.`;
}
...
async function generarTablaDatos() {
  try {
    ...
    const payload = {
      ...
      up:             "UP-07 DIRECCIÓN GENERAL DE ATENCIÓN AL MIGRANTE",
