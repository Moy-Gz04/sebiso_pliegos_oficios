...
const ADSCRIPCION_AREA = "UP-08 DIRECCIÓN GENERAL DE ASISTENCIA, ATENCIÓN Y PROTECCIÓN";   // ⚠️ CAMBIAR
...
function abrirModalSPG(codigo) {
  ...
  document.getElementById("spgUP").value     = "08";
...
async function abrirModalOficio2(codigo) {
  try {
    ...
    try {
      const responseOficios = await fetch(`${API}/api/presupuestos/ultimo-oficio/8`);
...
async function fetchRegistro(codigo) {
  ...
  const response = await fetch(`${API}/api/registros/UP-08`);
...
function construirOficioTablaDatos(numeroOficio) {
  return `SEBISO/DGAAP/${numeroOficio || ""}/2026.`;
}
...
async function generarTablaDatos() {
  try {
    ...
    const payload = {
      ...
      up:             "UP-08 DIRECCIÓN GENERAL DE ASISTENCIA, ATENCIÓN Y PROTECCIÓN",
