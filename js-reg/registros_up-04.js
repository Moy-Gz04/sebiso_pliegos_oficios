...
const ADSCRIPCION_AREA = "UP-04 DIRECCIÓN GENERAL DE FOMENTO ARTESANAL";   // ⚠️ CAMBIAR
...
function abrirModalSPG(codigo) {
  ...
  document.getElementById("spgUP").value     = "04";
...
async function abrirModalOficio2(codigo) {
  try {
    ...
    try {
      const responseOficios = await fetch(`${API}/api/presupuestos/ultimo-oficio/4`);
...
async function fetchRegistro(codigo) {
  ...
  const response = await fetch(`${API}/api/registros/UP-04`);
...
function construirOficioTablaDatos(numeroOficio) {
  return `SEBISO/DGFA/${numeroOficio || ""}/2026.`;
}
...
async function generarTablaDatos() {
  try {
    ...
    const payload = {
      ...
      up:             "UP-04 DIRECCIÓN GENERAL DE FOMENTO ARTESANAL",
