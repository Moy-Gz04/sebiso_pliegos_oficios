...
const ADSCRIPCION_AREA = "UP-05 SUBSECRETARÍA DE INCLUSIÓN Y DESARROLLO";   // ⚠️ CAMBIAR
...
async function abrirModalOficio2(codigo) {
  try {
    ...
    try {
      const responseOficios = await fetch(`${API}/api/presupuestos/ultimo-oficio/5`);
...
async function fetchRegistro(codigo) {
  ...
  const response = await fetch(`${API}/api/registros/UP-05`);
...
function construirOficioTablaDatos(numeroOficio) {
  return `SEBISO/SSID/${numeroOficio || ""}/2026.`;
}
...
async function generarTablaDatos() {
  try {
    ...
    const payload = {
      ...
      up:             "UP-05 SUBSECRETARÍA DE INCLUSIÓN Y DESARROLLO",
