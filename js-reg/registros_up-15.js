...
const ADSCRIPCION_AREA = "UP-15 SUBSECRETARÍA DE DESARROLLO SOCIAL Y HUMANO";   // ⚠️ CAMBIAR
...
function abrirModalSPG(codigo) {
  ...
  document.getElementById("spgUP").value     = "15";
...
async function abrirModalOficio2(codigo) {
  try {
    ...
    try {
      const responseOficios = await fetch(`${API}/api/presupuestos/ultimo-oficio/11`);
...
async function fetchRegistro(codigo) {
  ...
  const response = await fetch(`${API}/api/registros/UP-15`);
...
function construirOficioTablaDatos(numeroOficio) {
  return `SEBISO/SSDSyH/${numeroOficio || ""}/2026.`;
}
...
async function generarTablaDatos() {
  try {
    ...
    const payload = {
      ...
      up:             "UP-15 SUBSECRETARÍA DE DESARROLLO SOCIAL Y HUMANO",
