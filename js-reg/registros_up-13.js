...
const ADSCRIPCION_AREA = "UP-13 SUBSECRETARÍA DE PARTICIPACIÓN SOCIAL Y FOMENTO ARTESANAL";   // ⚠️ CAMBIAR
...
async function abrirModalOficio2(codigo) {
  try {
    ...
    try {
      const responseOficios = await fetch(`${API}/api/presupuestos/ultimo-oficio/9`);
...
async function fetchRegistro(codigo) {
  ...
  const response = await fetch(`${API}/api/registros/UP-13`);
...
function construirOficioTablaDatos(numeroOficio) {
  return `SEBISO/SPSyFA/${numeroOficio || ""}/2026.`;
}
...
async function generarTablaDatos() {
  try {
    ...
    const payload = {
      ...
      up:             "UP-13 SUBSECRETARÍA DE PARTICIPACIÓN SOCIAL Y FOMENTO ARTESANAL",
