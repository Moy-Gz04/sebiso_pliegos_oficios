...
const ADSCRIPCION_AREA = "UP-06 DIRECCIÓN GENERAL DE OPERACIÓN Y LOGÍSTICA DE PROGRAMAS";   // ⚠️ CAMBIAR
...
function abrirModalSPG(codigo) {
  ...
  document.getElementById("spgUP").value     = "06";
...
async function abrirModalOficio2(codigo) {
  try {
    ...
    try {
      const responseOficios = await fetch(`${API}/api/presupuestos/ultimo-oficio/6`);
...
async function fetchRegistro(codigo) {
  ...
  const response = await fetch(`${API}/api/registros/UP-06`);
...
function construirOficioTablaDatos(numeroOficio) {
  return `SEBISO/SSID/DGOLP/CI-${numeroOficio || ""}/2026.`;
}
...
async function generarTablaDatos() {
  try {
    ...
    const payload = {
      ...
      up:             "UP-06 DIRECCIÓN GENERAL DE OPERACIÓN Y LOGÍSTICA DE PROGRAMAS",
